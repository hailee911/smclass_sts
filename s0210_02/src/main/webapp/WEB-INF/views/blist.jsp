...
<meta charset="UTF-8">
  ...
  <script>
      const searchBtn = () =>{
		var serarchW = $()".searchW").val().trim(); //앞뒤공백제거
		if($(".searchW").val()==""){
...
<section>
    ...
    <c:if test="${page!=1}">
      ...
      <c:if test="${page!=maxpage}">
      <a href="/board/blist?page=${page+1}&category=${category}&searchW=${searchW}"><li class="next"></li>
      </a>
      </c:if>
      ...
      <c:if test="${page!=maxpage}">
      <a href="/board/blist?page=${endpage}&category=${category}&searchW=${searchW}"><li class="last"></li>
      </a>
      </c:if>
    </ul>
